...
void runShooter(Shooter *shooter)
{
	...
	shooterAtSpeed(shooter, speed);
}

void shooterAtSpeed(Shooter *shooter, int speed)
{
	bangBangChangeSetPoint((*(*shooter).controller), speed);
	int outputSpeed = runBangBangController((*(*shooter).controller), getIME((*shooter).ime));
